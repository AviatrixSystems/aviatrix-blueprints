...
d = Diagram(
    title="Zero-Trust MCP Egress: Obot on EKS with Aviatrix DCF",
...
d.add_container(Container(
    ...
    style="region", cloud="aws", corner_radius=6,
))

# Private subnets — LEFT (pods originate here)
d.add_container(Container(
    "priv-subnet", "Private subnets",
...
# Public subnet — RIGHT (gateway, internet-facing)
d.add_container(Container(
    "pub-subnet", "Public subnets",
...
# Aviatrix Gateway (PEP) — in public subnet on the RIGHT
d.add_node(Node(
    "gw", "",
    x=741, y=295,
...
# Internet Gateway (just outside VPC right edge)
d.add_node(Node(
...
# Gateway → IGW — egress exits VPC, goes to internet
# Simple horizontal: gw (741,295) → igw (960,295)
...
# Route: controller (960,435) → up to y=325 → left to gw (741,295)
# y=325 is below gateway center (295), enters from right
d.add_connection(Connection(
    "controller", "gw",
    color=COLORS['line'],
    thickness=1.2,
...
d.add_label(Label("m4", f"egress {SAFE_GLYPHS['arrow_right']} route table {SAFE_GLYPHS['arrow_right']} Gateway",
...
OUTPUT = "/Users/nickda/Code/avx-dev/aviatrix-blueprints/blueprints/obot-mcp-egress-aws/architecture.svg"
d.save(OUTPUT)
