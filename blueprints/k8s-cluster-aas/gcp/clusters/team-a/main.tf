...
#
# This was previously a thin wrapper around a non-existent
# `../../../../gcp-gke-multicluster/modules/gke-cluster` module. The module
# has been inlined here using google_container_cluster + supporting service
# accounts and IAM bindings, modelled on gcp-gke-multicluster/clusters/frontend.
#####################
...
locals {
  ...
  network_name   = data.terraform_remote_state.network.outputs.team_a_network_name
  subnet_name    = data.terraform_remote_state.network.outputs.team_a_gke_nodes_subnet_name
  pods_range     = data.terraform_remote_state.network.outputs.team_a_pod_range_name
  services_range = data.terraform_remote_state.network.outputs.team_a_services_range_name
  master_cidr    = data.terraform_remote_state.network.outputs.team_a_master_cidr
...
resource "google_container_cluster" "this" {
  ...
  # Manage the system node pool separately in nodes/team-a.
  ...
  # Dataplane V2 (Cilium / eBPF) — provides NetworkPolicy + observability.
  ...
  # Enable Gateway API for the L7 internal load balancer used by team apps.
  gateway_api_config {
...
#
# Bound to KSA kube-system/external-dns by the ExternalDNS Helm release in
# nodes/team-a. KSA -> GSA binding is created here so the cluster + IAM
# resources land together.
#####################
