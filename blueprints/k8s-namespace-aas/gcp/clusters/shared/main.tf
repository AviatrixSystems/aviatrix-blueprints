...
# get isolated namespaces within this cluster. Isolation enforced by DCF.
#
# Inlined from gcp-gke-multicluster/clusters/frontend (canonical pattern)
# because the previous `gke-cluster` module reference does not exist.
#####################
...
locals {
  ...
  network_name   = data.terraform_remote_state.network.outputs.shared_network_name
  subnet_name    = data.terraform_remote_state.network.outputs.shared_gke_nodes_subnet_name
  pods_range     = data.terraform_remote_state.network.outputs.shared_pod_range_name
  services_range = data.terraform_remote_state.network.outputs.shared_services_range_name
  master_cidr    = data.terraform_remote_state.network.outputs.master_ipv4_cidr_block
...
#####################
# Shared GKE Cluster (Control Plane Only — node pool managed in Layer 3)
#####################
