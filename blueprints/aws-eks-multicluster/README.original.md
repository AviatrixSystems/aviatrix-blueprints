...
├── network-aviatrix/           # Transit, Spokes, VPCs, DB VM, Route53

Layer 2: EKS Clusters (Control Plane Only)
├── eks/frontend-cluster/       # Control plane, IAM roles, security groups, VPC CNI addon
├── eks/backend-cluster/        # Control plane, IAM roles, security groups, VPC CNI addon

Layer 3: EKS Node Groups + Kubernetes Resources
├── eks/frontend-nodes/         # k8s-firewall Helm, ENIConfig, nodes, CoreDNS, Helm charts (ALB Controller, ExternalDNS)
├── eks/backend-nodes/          # k8s-firewall Helm, ENIConfig, nodes, CoreDNS, Helm charts (ALB Controller, ExternalDNS)
```
...
{"level":"info","ts":"2025-11-27T01:47:07Z","msg":"v1 Endpoints is deprecated in v1.33+; use discovery.k8s.io/v1 EndpointSlice"}
```
...
│   │       ├── main.tf         # Managed node group creation
│   │       ├── variables.tf
│   │       └── outputs.tf
│   │
│   ├── frontend-cluster/       # Deploy Layer 2 for frontend
│   │   ├── main.tf             # Cluster control plane, IAM, security groups
...
```
network-aviatrix/terraform.tfstate
    │
    ├── eks/frontend-cluster/terraform.tfstate
    │       │
    │       └── eks/frontend-nodes/terraform.tfstate
    │
    └── eks/backend-cluster/terraform.tfstate
            │
            └── eks/backend-nodes/terraform.tfstate
```
...
- [CLAUDE.md](CLAUDE.md) - Detailed guidance for AI assistants working with this repository
...
5. Document breaking changes in VERSION_UPDATES.md
