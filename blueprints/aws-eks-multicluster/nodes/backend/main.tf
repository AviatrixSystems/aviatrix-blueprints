...
resource "kubernetes_manifest" "eniconfig" {
  for_each = { for idx, az in data.terraform_remote_state.network.outputs.backend_availability_zones : az => data.terraform_remote_state.network.outputs.backend_pod_private_subnet_ids[idx] }
...
#####################
# Backend EKS Node Group
#####################

# This deployment runs AFTER backend-cluster exists
