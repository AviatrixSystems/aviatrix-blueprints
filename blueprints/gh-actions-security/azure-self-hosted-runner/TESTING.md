# Testing Guide — Azure Self-Hosted Runner + Aviatrix DCF
...
Actions self-hosted runner running on an Azure VM in an Azure spoke VNet.
...
| Requirement | Detail |
|---|---|
| Self-hosted runner deployed | `azure-self-hosted-runner` blueprint applied; runner shows **online** in `Settings → Actions → Runners` |
...
|---|---|---|---|
| 10 | `deny-*-threat-group` | DENY (enforced) | Runner VM → Aviatrix ThreatIQ feed |
| 20 | `allow-*-github` | PERMIT | Runner VM → GitHub FQDNs (TCP 443) |
| 30 | `allow-*-tool-calls` | PERMIT | Runner VM → `www.example.com` (TCP 80/443) |
| 40 | `allow-*-linux-setup` | PERMIT | Runner VM → Ubuntu APT FQDNs (TCP 80/443) |
| **50** | **`deny-*-unmatched-web`** | **DENY + watch** | Runner VM → All-Web (TCP 80/443) |
...
| Source | Runner VM IP (`10.10.10.x`) |
...
2. Run the same workflow (same webhook URL, cloud=azure, retries=3).
